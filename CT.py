...
modelname = "CT_R30_aniso_0.99_ell_0.6_meshsize_0.2"

meshname  = modelname+"-mesh.xdmf"

savedir   = "CT_results_ell_0.6/"+modelname+"/"
...
#Geometry
L= 50.; h=48.; x_centre= -15.; y_centre1=11; y_centre2=-11; a0=30;rayon_f=5.2;
...
crack_w = 0.005*L*tan(crack_angle)
...
circle1 = Circle(Point(x_centre, y_centre1), rayon_f)
circle2 = Circle(Point(x_centre, y_centre2), rayon_f)
...
class X0(SubDomain):
    def inside(self, x, on_boundary):
        return(on_boundary and ((x[0]-x_centre)**2 + (x[1]-y_centre1)**2 < (rayon_f+eps)**2) and (x[1]>=y_centre1+coef*rayon_f+eps))

    
class X2(SubDomain):
    def inside(self, x, on_boundary):
        return (on_boundary and ((x[0]-x_centre)**2 + (x[1]-y_centre2)**2 < (rayon_f+eps)**2) and (x[1]<=y_centre2-coef*rayon_f+eps))
...
ell = Constant(0.012) # 0.4 mm /50 mm
ut = Constant(1.0)
...
# Boundary condition for damage field
